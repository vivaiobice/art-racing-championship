(function(root,factory){
  ...
  function rankDrivers(drivers){return [...drivers].sort(compareDrivers);}
  function filterDrivers(drivers,query){const needle=String(query||"").trim().toLocaleLowerCase("it-IT");if(!needle)return drivers;return drivers.filter(driver=>[driver.name,driver.psn,driver.number,driver.team].some(value=>String(value??"").toLocaleLowerCase("it-IT").includes(needle)));}
  function racePoints(results,raceNumber){return results.filter(result=>result.race===raceNumber).sort((a,b)=>(a.pos??999)-(b.pos??999)).map(result=>{const noteStatus=String(result.note||"").trim().toUpperCase().match(/^(FINISH|DNF|DNS|DSQ)\b/)?.[1];return {...result,displayStatus:normalizeResultStatus(noteStatus||result.status,result.present)};});}
  ...
  function applyStats(piloti,gare,qualifiche,risultati,classifiche){
    ...
    const sorted=rankDrivers([...byId.values()]);
  ...
  return {PUBLIC_ID,GIDS,ENDPOINTS,parseCSV,normalizeNumber,normalizeBoolean,normalizeResultStatus,roundPoints,calculateChampionship,compareDrivers,rankDrivers,filterDrivers,racePoints,mapPiloti,mapGare,mapQualifiche,mapRisultati,mapClassifiche,mapConfig,applyStats,createClient,...defaultClient};
});
